...
af2[#af2+1] = Def.ActorFrame{
	...
	LoadFont(ThemePrefs.Get("ThemeFont") .. " Normal")..{
		...
		InitCommand=function(self)
			...
			self:maxwidth(width/textZoom-10):zoom(textZoom):maxheight(12):y(-1)
		...
		RedrawCommand=function(self)
			...
			if GAMESTATE:GetCurrentSteps(player):IsAutogen() then self:maxheight(10):settext(THEME:GetString("ScreenSelectMusic", "AUTOGENBREAKDOWN")) end
		end,
	...
	LoadFont(ThemePrefs.Get("ThemeFont") .. " Normal")..{
		...
		InitCommand=function(self)
			local textZoom = 0.8
			--let's give some padding so the text doesn't touch the outer edges of this box
			self:maxwidth(width/textZoom-10):zoom(textZoom):diffusealpha(0):y(-1)
			self:queuecommand("MarqueeFlash")
		end,
		HideCommand=function(self)
			self:settext("")
