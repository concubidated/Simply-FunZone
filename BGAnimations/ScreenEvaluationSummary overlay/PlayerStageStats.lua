...
-- stepartist
-- FIXME: show which charts are autogenerated; can't pass GAMESTATE:GetCurrentSteps(player):IsAutogen() here because GAMESTATE has no current steps
-- all stepartists are packaged into the local variable "stepartist"
af[#af+1] = LoadFont(ThemePrefs.Get("ThemeFont") .. " Normal")..{
...
af[#af+1] = Def.ActorProxy{
	...
	DrawStageCommand=function(self)
		if playerStats and grade then
			local grade_actor = grade
			if playerStats.judgments and playerStats.judgments.W0 and playerStats.exscore == 100 then
				grade_actor = "Grade_Tier00"
			end

			local target = LetterGradesAF and LetterGradesAF:GetChild(grade_actor)
			if target then
				self:SetTarget(target):visible(true)
			else
				self:visible(false)
			end
		else
